...
def notifyBle(self, handle, data):
    ...
    read_data = device.char_read("00002a6e-0000-1000-8000-00805f9b34fb")
    print(read_data)
...
try:
    adapter.start()
    #device = adapter.connect('93:89:C7:2D:5F:12', address_type=pygatt.BLEAddressType.random)
    device = adapter.connect('93:89:C7:2D:5F:12')
    #device.subscribe(uuid.UUID(temp_characteristic), callback=notifyBle)
